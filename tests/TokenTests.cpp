...
suite token_tests = [] {
	...
	"Named empty"_test = [] { expect(Token{ {}, L"a" } != Token{}); };
	"Equals"_test = [] { expect(Token{ Token::Type::Identifier, L"a" } == Token{ Token::Type::Identifier, L"a" }); };
	"Unequal type"_test = [] { expect(Token{ Token::Type::Identifier, L"a" } != Token{ Token::Type::LiteralString, L"a" }); };
	"Unequal name"_test = [] { expect(Token{ Token::Type::Identifier, L"a" } != Token{ Token::Type::Identifier, L"b" }); };
	"Compare empty to false"_test = [] { expect(Token{} == false); };
	"Compare named empty to false"_test = [] { expect(Token{ {}, L"a" } == false); };
