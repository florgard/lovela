...
suite to_int_tests = [] {
	...
	static_assert(to_int<int>("234").unsignedValue.value_or(123) == 234, "Arbitrary number");
	...
	static_assert(!to_int<int64_t>("18446744073709551616").has_value(), "Larger than 64 bit can't be converted");
	static_assert(!to_int<int64_t>("20000000000000000000").has_value(), "Arbitrary width, positive big int can't be converted");
